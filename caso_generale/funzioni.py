...
def scelta_sorgente_radioattiva() :

    """
    Descrizione
    -----------

        funzione che non ha nulla per input ma che mi restituisce l'energia cinetica delle particelle per l'esperimento in base al tipo di sorgente scelta.
        vengono date a disposizione delle sorgenti predefinite ma l'utente può scegliere a piacere l'energia cinetica in Mev delle particelle, 
        senza specificare il tipo di sorgente.
    ...
    -----------
        ...
        energia in Mev delle particelle della sorgente 
    """

    #inizializzo una variabile

    energia = 4.257
    sorgente = "Uranio-238r23523"
    
    #creo le varie tabelle radioattive

    sorgenti_radioattive = [ "Uranio-238" , "Thorium-232" , "Radon-222" , "Polonio-210" , "Plutonio-239" , "Americium-226" , "Radium-226" , "Radon-220" , "Francium-223" ]
    energia_in_mev = [ 4.2 , 4.0 , 5.5 , 5.4 , 5.1 , 5.5 , 4.8 , 6.0 , 4.9 ]

    #faccio vedere tutte le sorgenti radioattive a disposizione

    print( "si hanno a disposizione le seguenti sorgenti radioattive: " )

    for i in range( 0 , len( sorgenti_radioattive ) , 1 ) :
        print( sorgenti_radioattive[i] , "  con energia pari ad " , energia_in_mev[i] ,  " MeV" )


    #do libera scelta se voglio utilizzare una sorgente tra queste a disposizione o se voglio inserire a piacere l'energia in Mev delle particelle alpha

    modifica = "si"

    while( modifica == "si" ) :

        scelta = input( "\nscrivere 'si' se si vuole utilizzare una delle sorgenti radioattive messe a disposizione.\nscrivere 'no' se si vuole inserire a piacere l'energia in MeV delle particelle:  ")
       
        if scelta == "si":

            sorgente = input("\nscrivere il nome della sorgente radioattiva che si vuole utilizzare:  " )

            #creo un ciclo for che mi controlli se il nome scelto coincide con qualcuno delle sorgenti radioattive e salvo il valore dell'energia

            while sorgente not in sorgenti_radioattive :
                    
                sorgente = input("\nhai sbagliato a scrivere il nome della sorgente.\nriprovare: ")    
                
            energia = energia_in_mev[i]
                
            print( "\nsi è quindi scelto l'elemento " , sorgente , " che ha un'energia pari a " , energia , " MeV" )
        ...
        if scelta == "no" :
            ...
            print( "\nl'energia scelta per le particelle è: " ,  energia , " MeV\n" )
